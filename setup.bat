...
REM Immer "python.exe -m pip" statt "pip.exe" verwenden: pip.exe ist ein
REM Launcher mit fest eingebackenem absolutem venv-Pfad und bricht, sobald
REM das Repo-Verzeichnis umzieht. python.exe -m pip ist pfadunabhaengig.
"%~dp0.venv\Scripts\python.exe" -m pip install --upgrade pip --quiet %PIP_TRUSTED%
"%~dp0.venv\Scripts\python.exe" -m pip install --upgrade -e "%~dp0." --quiet %PIP_TRUSTED%
...
"%~dp0.venv\Scripts\python.exe" -m pip install --force-reinstall --no-deps --quiet %PIP_TRUSTED% "git+https://github.com/michaelblaess/textual-themes.git"
